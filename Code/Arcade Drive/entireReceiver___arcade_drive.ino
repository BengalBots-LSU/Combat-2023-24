...
const int leftMotor = 24; //saber motor controller
const int rightMotor = 25; //saber motor controller
const int weaponMotor = 26; //talon motor controller
const int kSwitch = 9; //kill switch 
const int greenLED = 5;
const int leftMid = 511;
const int rightMid = 502;
Servo motorRight, motorLeft, motorWeapon;
...
{
  motorLeft.write(x > leftMid ? x : buttonMax - x);
  motorRight.write(y > rightMid ? y : buttonMax - y);
}
...
void setup() {
  ...
  motorRight.attach(rightMotor, 1000, 2000);
  motorLeft.attach(leftMotor, 1000, 2000);
  motorWeapon.attach(weaponMotor, 1000, 2000);
  //joystick[0] = map(data.jDirectionR, 0, buttonMax, 0, 180);
  //joystick[1] = map(data.jDirectionL, 0, buttonMax, 180, 0);

  //motorRight.attach(rightMotor); 
  //motorLeft.attach(leftMotor);
}
...
void loop() {
  ...
  while(radio.available()) {
    ...
    digitalWrite(weaponMotor, data.killButton ? HIGH : LOW);
